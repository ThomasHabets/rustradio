...
use crate::stream::{Stream, Tag, TagPos, TagValue};
use crate::{Error, Sample};

/// Stream to PDU block.
pub struct StreamToPdu<'a, T> {
    src: &'a Stream<T>,
    dst: &'a Stream<Vec<T>>,
    tag: String,
    buf: Vec<T>,
...
impl<T> Block for StreamToPdu<'_, T>
where
    T: Copy + Sample,
{
