...
pub struct SigMFSourceBuilder<T: Copy + Type + Send + Sync + 'static> {
...
impl<T: Default + Copy + Type + Send + Sync + 'static> SigMFSourceBuilder<T> {
...
#[rustradio(crate)]
pub struct SigMFSource<T: Copy + Send + Sync + 'static> {
    file: std::fs::File,
    meta: SigMF,
    range: (u64, u64),
    left: u64,
...
impl<T: Default + Copy + Type + Send + Sync + 'static> SigMFSource<T> {
...
where
    T: Sample<Type = T> + Copy + std::fmt::Debug + Type + Send + Sync + 'static,
{
