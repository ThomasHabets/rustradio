...
use crate::stream::{new_streamp, ReadStreamp,ReadStream,Streamp, Tag, TagValue};
...
impl<T> BurstTagger<T> {
    ...
    /// * tag: Tag name to add.
    pub fn new(src: ReadStreamp<T>, trigger: Streamp<Float>, threshold: Float, tag: String) -> Self {
        Self {
            src,
            trigger,
            threshold,
